...
- `idCollectionItemName` (Text) — Collection Item Name [hidden]

## XML Information

- `SdtItemXmlType` (Combo) — XmlType. Default: `Element`. Values: Attribute, CData, Element, Value
- `idXmlName` (Text) — XML Name
- `idXmlCollectionItemName` (Text) — XML Collection Item Name [hidden]
- `idXmlNamespace` (Text) — XML Namespace
- `soaptype` (Text) — SoapType. Soap Type
- `idXmlInclude` (Combo) — XML Null Serialization. Values: Empty Tag, No Tag, Nil Tag
- `idWRAPPEDCOLLECTION` (Combo) — Collection Serialization. Default: `idXmlCollectionWrapped`. Values: Wrapped, Sequence [hidden]
...
- `idJsonName` (Text) — Json Name
- `idJsonInclude` (Combo) — Json Null Serialization. Select whether a null value should be serialized as empty or not at all.. Values: Empty, No Property, JSON null
- `idWRAPPEDJSONCOLLECTION` (Combo) — Json Collection Serialization. Specify the way collections are serialized. Default: `idXmlCollectionWrapped`. Values: Wrapped, Sequence [hidden]
