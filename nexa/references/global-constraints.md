...
- Property names in `.gx` files must use CamelCase without spaces or quotes; when a property is documented with spaces (e.g., `"Maximum numeric length"`), convert it by removing spaces and capitalizing each word: `"Maximum numeric length"` (✘) → `MaximumNumericLength` (✓), `"Business Component"` (✘) → `BusinessComponent` (✓)
